...
define(function(require, exports, module) {
    ...
    var CalendarHeatMap = SimpleSplunkView.extend({
        ...
        heatmapOptionNames: [
            ...
            'onMinDomainReached', 'width', 'height','displayLegend'],
        ...
        updateView: function(viz, data) {     
            ...
            _.each(data.series, function(series, idx) {
                ...
                var $title = $("<h4 class='heatmap-series-title'></h4>").appendTo($el);
                ...
                var options = _.extend({
                    ...
                    onClick: function(date, value) {
                        var tabId = that.options.tabId;
                        var master = that.options.master;
                        var slave = that.options.slave;

                        if (value == 0) {
                            if (!mvc.Components.getInstance(tabId + '-offline-reason-search')) {
                                new SearchManager({
                                    id: tabId + "-offline-reason-search"
                                   });
                            }
                            if (!mvc.Components.getInstance(tabId + '-offline-reason-table')) {
                                var slaveOfflineReasonTable = new TableView({
                                    id: that.options.tabId + "-offline-reason-table",
                                    managerid: that.options.tabId + "-offline-reason-search",
                                    pageSize: 15,
                                    pagerPosition: top,
                                    drilldown: 'none',
                                    drilldownRedirect: false,
                                    el: $('#' + tabId + '-connection-content .slave-build-connection-table')
                                }).render();
                                Util.addJenkinsSlaveCustomIconRenderer(slaveOfflineReasonTable);
                            }

                            var earliest_time = moment(date.valueOf(), 'x').format("MM/DD/YYYY:HH:mm:ss");
                            var latest_time = moment((date.valueOf() + 3600000), 'x').format("MM/DD/YYYY:HH:mm:ss");
                            var search = 'index=jenkins_statistics earliest=' + earliest_time + ' latest=' + latest_time +
                                '  host="' + master + '" node_name="' + slave + '" event_tag="slave" event_src="monitor" ' +
                                ' |table _time, host, node_name, is_online,  offline_reason, url' +
                                ' |rename host as "Jenkins Master", node_name as "Jenkins Slave", is_online as "Online", offline_reason as "Offline Reason", url as "Slave URL"';
                            mvc.Components.getInstance(tabId + "-offline-reason-search").settings.set("search", search);
                        }
                    }
                }, vizOptions);
                ...
                cal.options.subDomainTitleFormat = {
                    empty: "{date}",
                    filled: "{date}"
                };
